...
class Msgs extends Model
{
    ...
    public function createTouch($uId,$ulId,$hId){
        ...
        if(isset($hId)){
            ...
            if($hId == 0){
                ...
                //$houseAddr = $adminInfo['street'].' '.$adminInfo['address'];
                ...
                if($adminInfo['is_admin'] == 2){
                    ...
                    if($isRepeat['mp_id']){
                        ...
                        if($msg['xcx_msg_ul_type'] == 2){
                            $mpid = $isRepeat['mp_id'];
                            $content = $msg['xcx_msg_content'];
                            $title = $houseAddr;
                            $msgs = new Msg();
                            $msgs->sendEmail($mpid, $uId, $ulId, $content,$title,0);
                        }*/
                        return $isRepeat['mp_id'];
                    }else{
                        //写入一条创建会话的记录
        ...
        //前端对前端发送消息
        $isRepeat = Db::table('xcx_msg_person')
            ->where("(mp_u_id = ".$uId." and mp_utype = 1 and mp_ul_id = ".$ulId." and mp_ultype = 1 and mp_isable = 1 ) or (mp_ul_id = ".$uId." and mp_utype = 1 and mp_u_id = ".$ulId." and mp_ultype = 1 and mp_isable = 1 )")
            ->field('mp_id')
            ->find();
        if($isRepeat['mp_id']){
            //更新会话时间
    ...
    public function getHouseAddAdminIdViaHouseId($hid){
        $houseInfo = Db::table('tk_houses')
            ->where(['id' => $hid])
            ->field('is_admin,user_id,pm,title,dsn,address,street')
            ->find();
