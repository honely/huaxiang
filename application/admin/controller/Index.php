...
class Index extends Controller
{
    ...
    public function index(){
        ...
        $buNum=Db::table('crm_user_alert')
            ->where(['alert_admin' => $adminId])
            ->where('alert_time','between',[$stime,$etime])
            ->order('alert_time desc')
            ->count();
        $buNum = $buNum >=100 ? '99+': $buNum;
        //表单客户
        $form = Db::table('crm_form_user')
            ->where(['status' => 2 ,'type' =>1])
            ->count();
        $this->assign('formUser',$form);
        $this->assign('buNum',$buNum);
        return  $this->fetch();
    }
    ...
    public function welcome(){
        ...
        $this->assign('buNum',$buNum);
        //看房单
        $kanfang = Db::table('crm_order')->where(['order_step' =>1,'order_status' =>1])->field('crm_id')->count();
        $this->assign('kanfang',$kanfang);
        //水电气网
        $energy = Db::table('crm_order')->where(['order_step' =>3,'order_status' =>1])->field('crm_id')->count();
        $this->assign('energy',$energy);
        //清洁检查
        $now = date('Y-m-d');
        $tomorrow = date("Y-m-d",strtotime("+1 day"));
        $clean = Db::table('crm_clean_log')
             ->where('cl_date','between',[$now,$tomorrow])
            ->where(['cl_status' =>3])
            ->field('cl_id')->count();
        //待审房源
        $house = Db::table('tk_houses')->where(['status' =>3])->field('id')->count();
        $show = 0;
        if($roleId == 38){
            $show = 1;
        }elseif($roleId == 2 || $roleId == 3){
            $show = 2;
        }elseif ($roleId == 33 || $roleId == 35){
            $show = 3;
        }
